...
//    NSArray *letters = [NSArray arrayWithObjects:@"A", @"B", @"C", @"D", nil];
    ...
    self.dial4 = [[[DialController alloc] initWithDialFrame:CGRectMake(DIAL_OFFSET_X + dialCount++ * DIAL_WIDTH, DIAL_OFFSET_Y, DIAL_WIDTH, DIAL_HEIGHT) strings:numbers] autorelease];
...
{
	NSString *str = [NSString stringWithFormat:@"%04d", number];
	[self.dial1 spinToString:[str substringWithRange:NSMakeRange(0, 1)]];
	[self.dial2 spinToString:[str substringWithRange:NSMakeRange(1, 1)]];
	[self.dial3 spinToString:[str substringWithRange:NSMakeRange(2, 1)]];
	[self.dial4 spinToString:[str substringWithRange:NSMakeRange(3, 1)]];
}
